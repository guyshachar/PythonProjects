...
class RefPortalApi():
    def __init__(self):
        # Configure logging
        logLevel = eval(f"logging.{os.getenv('logLevel') or 'DEBUG'}")
        ...
        self.twilioFromMobile = os.getenv('twilioFromMobile')
        self.twilioClient = TwilioClient(twilioServiceId=twilioServiceId, fromMobile=self.twilioFromMobile)
        self.twilioOnBoardingAdminMobile = os.getenv('twilioOnBoardingAdminMobile')

        openText=f'Ref Portal Api {helpers.localNow().strftime("%Y-%m-%d %H:%M:%S")} build#{os.getenv("BUILD_DATE")} host={socket.gethostname()}'
        self.logger.info(openText)
        
        print('create app')
        self.app = Flask(__name__)
        self.limiter = Limiter(self.home, app=self.app, default_limits=[f'{os.getenv("limitPerMin")} per minute'])
    ...
    def start(self):
        try:
            ...
            ssl_file_path = f'{os.getenv("MY_SSL_FOLDER", "/run/ssl/")}'
            flaskDebug = eval(os.getenv('flaskDebug') or 'True')
