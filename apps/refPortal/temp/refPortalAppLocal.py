...
class RefPortalApp():
    ...
    async def initialize_pages(self, num_pages):
        """
        Create and add pages to the queue.
        """
        for _ in range(num_pages):
            page = await self.browser.new_page()  # Create a new page
    ...
    async def acquire_page(self):
        """
        Acquire a semaphore and its associated page.
        """
    ...
    async def release_page(self, semaphore, page):
        """
        Release a semaphore and return it to the queue.
        """
    ...
    async def checkRefereeTask(self, browser, referee):
        page = None
        try:
    ...
    async def start(self):
        ...
        try:
            async with async_playwright() as p:
                ...
                self.browser = await p.chromium.launch(headless=True, args=['--disable-dev-shm-usage','--disable-extensions','--no-sandbox','--disable-setuid-sandbox','--disable-gpu','--disable-software-rasterizer','--verbose'])
                ...
                #await self.initialize_pages(5)
                pg = await self.browser.new_page()
                ...
                while True:
                    try:
                        refereesTasks = []
                        for referee in self.referees:
                            await self.checkRefereeTask(browser, referee)
                            #refereesTasks.append(asyncio.create_task(self.checkRefereeTask(browser, referee)))
